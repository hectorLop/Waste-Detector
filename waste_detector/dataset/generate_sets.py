...
from waste_detector.dataset.utils import split_data, add_background_imgs, split_data_classifier
...
def generate_sets(config : Dict):
    if len(config['annotations']) == 1:
        annotations_df, categories_df = process_unique_annotations(config['annotations'][0])
    else:
